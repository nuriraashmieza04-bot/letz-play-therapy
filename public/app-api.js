...
(function () {
  ...
  /* ============================================================
     AUTH
     ============================================================ */
  function visibleError(title, message) {
    try { toast(title, message, 'warn'); } catch (_) {}
    setTimeout(() => {
      const loginVisible = document.getElementById('login');
      if (loginVisible && loginVisible.style.display !== 'none') alert(title + ': ' + message);
    }, 50);
  }
  ...
  window.doLogin = async function () {
    ...
    try {
      ...
      visibleError('Sign in failed', e.message);
    } finally {
      if (btn) { btn.disabled = false; btn.textContent = oldText || 'Sign in to workspace'; }
    }
  };

  window.apiSubmitAuth = function (e) {
    if (e) {
      e.preventDefault();
  ...
  window.submitAuth = window.apiSubmitAuth;

  function bindAuthControls() {
    const btn = document.getElementById('authBtn');
    if (btn && !btn._apiAuthBound) {
      btn.addEventListener('click', (e) => {
        window.apiSubmitAuth(e);
      });
  ...
  document.addEventListener('click', (e) => {
    if (e.target && e.target.closest && e.target.closest('#authBtn')) {
      window.apiSubmitAuth(e);
    }
  }, true);
  if (document.readyState === 'loading') window.addEventListener('DOMContentLoaded', bindAuthControls);
  else bindAuthControls();
